...
contains
    ...
    character function process_long( longopts, arg )
                ...
                if ( longopts(i)%has_arg ) then
                    if (has_equalsign) then ! long option has equal sign between value and option
                        if (arg(len_arg+2:) == '') then ! no argument (len_arg+2 value after "="
                            write(*, '(a,a,a)') "Error: option '", trim(arg), "' requires an argument"
        ...
        process_long = '?'
        if ( opterr ) then
            write(*, '(a,a,a)') "Error: unrecognized option '", trim(arg), "'"
        endif
    ...
    character function process_short( optstring, arg )
        ...
        if ( i == 0 ) then
            ...
            process_short = '?'
            if ( opterr ) then
                write(*, '(a,a,a)') "Error: unrecognized option '-", optopt, "'"
            endif
        endif
        if ( i > 0 .and. substr( optstring, i+1, i+1 ) == ':' ) then
            ...
            elseif ( optind <= command_argument_count()) then
                ...
                optind = optind + 1
            elseif ( opterr ) then
                write(*, '(a,a,a)') "Error: option '-", optopt, "' requires an argument"
